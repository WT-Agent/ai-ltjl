  ...
  <div class="app-container">
    ...
    <main ref="inputCardRef" class="glass-card input-group">
      ...
      <div class="options-row" style="display: grid; grid-template-columns: repeat(auto-fit, minmax(200px, 1fr)); gap: 1rem;">
        <div class="selector-group">
          <label class="selector-label">关系定位</label>
          <div class="style-selector">
            <button 
        ...
        <div class="selector-group">
          <label class="selector-label">沟通疑惑</label>
          <div class="style-selector">
            <button 
      ...
      <div class="selector-group">
        <div style="display: flex; justify-content: space-between; align-items: center;">
          ...
          <div style="display: flex; gap: 0.5rem;">
            ...
            <button class="text-link-btn" @click="showGuideModal = true">微信聊天心理解读与高情商破局指南</button>
          </div>
        </div>
        <textarea 
          v-model="userInput" 
...
const appTitle = ref(appConfig.title || '网腾无限AI - 聊天记录分析与隐意解读专家');
...
const metricsList = [
  {
    "key": "hiddenSubtextInsight",
    "label": "潜台词隐意洞察度"
  },
  {
    "key": "psychologicalAttitudeAccuracy",
    "label": "心理态度评估准确度"
  },
  {
    "key": "interactionRhythmPacing",
    "label": "互动节奏掌控力"
  },
  {
    "key": "eqStrategyPracticality",
    "label": "高情商应对方案实用度"
  },
  {
    "key": "relationshipBoundaryWarning",
    "label": "关系边界预警分值"
  }
];
const guideRules = [
  {
    "title": "关注字数与时延",
    "advice": "对方回复字数、表情符号使用频率及回复时间间隔是衡量心理投入的关键指标",
    "avoid": "切忌对单字回复进行过度过度解读产生内耗"
  },
  {
    "title": "识别客套与真诚",
    "advice": "礼貌的问候往往代表客套边界，而分享个人生活细节与情绪代表信任拉近",
    "avoid": "切勿把对方礼貌性的客套当成深入发展的信号"
  },
  {
    "title": "掌控互动能量",
    "advice": "聊天能量应保持基本对等，避免输出长篇大论而对方仅回复一两个字",
    "avoid": "严禁在对方明显冷淡时进行连环问话轰炸"
  }
];
...
const displayResultText = computed(() => {
  ...
  return result.value.replace(/\[LTJL_SCORES\][\s\S]*?\[\/LTJL_SCORES\]/g, '').trim();
});

const parseAiScores = (rawText: string) => {
  const match = rawText.match(/\[LTJL_SCORES\](.*?)\[\/LTJL_SCORES\]/);
